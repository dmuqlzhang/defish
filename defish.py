...
class Defisheye:
    ...
    fov: fisheye field of view (aperture) in degrees 相机可以接收影像的角度范围，也可以常被称为视野
    ...
    def __init__(self, infile, **kwargs):
        ...
        if type(infile) == str:
            ...
            _image = cv2.imdecode(np.fromfile(infile,dtype=np.uint8),-1)
    ...
    def convert(self, outfile):
        ...
        # width = img.shape[1]
        # height = img.shape[0]
        # p1 = np.float32([[0, 0], [width - 1, 0], [0, height - 1], [width - 1, height - 1]])
        # p2 = np.float32(
        #     [[0, 0], [width-180, 181], [0, height-1], [width - 181, height - 181]])
        # M = cv2.getPerspectiveTransform(p1, p2)
        # img = cv2.warpPerspective(img, M, (width, height))
        # cv2.namedWindow("1", 0)
        # cv2.resizeWindow("1", 480, 480)
        # cv2.imshow('1', img)
        # cv2.waitKey()
        img = cv2.resize(img, (960, 960))
        cv2.imencode('.jpg', img)[1].tofile(outfile)
